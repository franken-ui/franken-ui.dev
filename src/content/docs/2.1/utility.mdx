...
It's often used within the [Modal component](/docs/2.1/modal).

<Ec path="utility.1" />

<Snippet component="modal.overflow" />

You can change the target heights by adding the `selContainer` and `selContent` options to the attribute. [Learn more](/docs/2.1/javascript#component-configuration)
...
To modify the origin of an animation, like scaling, add one of the `uk-transform-origin-*` classes. This can be combined with the [Animation component](/docs/2.1/animation).
