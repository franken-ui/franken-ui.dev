...
Apply one or more animations to the dropdown by adding the `animation: uk-anmt-*` option with one of the classes from the [Animation component](/docs/2.1/animation). You can also determine the animation's duration. Just add the `duration` option with a value in milliseconds. Set `animate-out: true` to also show an animation when closing the drop.
